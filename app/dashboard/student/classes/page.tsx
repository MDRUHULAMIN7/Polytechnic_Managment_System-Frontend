...
export default async function StudentClassesPage({ searchParams }: PageProps) {
  ...
  const payload = semesterRegistration
    ? await getStudentClassSessionsServer({
        semesterRegistration,
  ...
  return (
    <section className="space-y-6">
      <DashboardPageHeader
        title="My Classes"
        description="Start from your enrolled semester registration, then inspect class status and attendance for that term only."
      />
      ...
      {!semesterRegistration ? (
        <div className="rounded-2xl border border-dashed border-(--line) bg-(--surface) px-5 py-10 text-center">
          <p className="text-sm font-medium">Select a semester registration to load your class list.</p>
          <p className="mt-2 text-sm text-(--text-dim)">
            This keeps your dashboard focused on the semester you are currently reviewing.
          </p>
        </div>
      ) : null}
              ...
              >
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm">{item.myAttendance?.status ?? "Not marked"}</p>
                  </div>
                </div>

                <Link
                  href={`/dashboard/student/classes/${item._id}${detailQuery ? `?${detailQuery}` : ""}`}
