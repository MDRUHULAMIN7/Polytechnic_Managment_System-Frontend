...
import { getAdminClassSessionsServer } from "@/lib/api/dashboard/class-session/server";
import { PageProps } from "@/lib/type/dashboard/admin/type";
import { parseNumberParam, readParam, resolveName } from "@/utils/dashboard/admin/utils";
import {
  formatClassDate,
  formatTimeRange,
...
function buildPageHref(
  ...
  return `/dashboard/admin/classes?${query.toString()}`;
}
...
export default async function AdminClassesPage({ searchParams }: PageProps) {
  ...
  const payload = await getAdminClassSessionsServer({
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-(--text-dim)">
            Review schedule, instructor delivery, and attendance outcome with
            paginated results.
          </p>
        </div>
      ...
      <form
        action="/dashboard/admin/classes"
              ...
              <tr key={item._id} className="border-b border-(--line)/70">
                ...
                <td className="px-4 py-3 text-(--text-dim)">
                  {typeof item.instructor === "string"
                    ? item.instructor
                    : `${resolveName(item.instructor?.name)} (${item.instructor?.designation ?? "--"})`}
                </td>
