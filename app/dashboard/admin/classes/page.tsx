...
import { formatClassDate, formatTimeRange, statusBadgeClass } from "@/utils/dashboard/class-session";
    ...
    <section className="space-y-6">
      ...
      {!semesterRegistration ? (
        <div className="rounded-2xl border border-dashed border-(--line) bg-(--surface) px-5 py-10 text-center">
          <p className="text-sm font-medium">Select a semester registration to load classes.</p>
          <p className="mt-2 text-sm text-(--text-dim)">
            This keeps the table focused on one term and avoids loading mixed-semester data.
          </p>
        </div>
      ) : null}
          ...
          <div className="space-y-3 md:hidden">
            {payload.result.map((item) => (
              ...
              >
                ...
                <Link
                  href={`/dashboard/admin/classes/${item._id}${detailQuery ? `?${detailQuery}` : ""}`}
