    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-(--line) bg-(--surface) p-4 lg:flex-row lg:items-end lg:justify-between">
      <div className="w-full sm:max-w-sm">
        ...
        <input
          ...
          placeholder="Search by title, prefix, or code"
      ...
      <div className="grid w-full gap-3 sm:grid-cols-2 lg:w-auto">
        ...
        <div className="min-w-44">
          ...
          <select
            value={sort}
            onChange={(event) =>
              onSortChange(event.target.value as SubjectFiltersProps["sort"])
            }
            className="focus-ring mt-2 h-11 w-full rounded-xl border border-(--line) bg-(--surface) px-3 text-sm text-(--text)"
          >
            <option value="-title">Title Z-A</option>
            <option value="title">Title A-Z</option>
            <option value="-code">Code High-Low</option>
            <option value="code">Code Low-High</option>
          </select>
        </div>
      </div>
    </div>
