...
type AcademicDepartmentTableProps = {
  loading: boolean;
  pagination: PaginationResult<AcademicDepartment>;
  onDetails: (row: AcademicDepartment) => void | Promise<void>;
  onUpdate: (row: AcademicDepartment) => void;
  onPrevPage: () => void;
  onNextPage: () => void;
};
    ...
    <>
      <div className="mt-4">
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-(--line) p-8 text-center text-sm text-(--text-dim)">
            No academic department found for current filters.
          </div>
        ) : (
          <>
                  ...
                  {pagination.items.map((row) => (
                    <tr key={row._id} className="border-b border-(--line) last:border-b-0">
                      ...
                      <td className="px-3 py-3 text-(--text-dim)">
                        {resolveAcademicDepartmentInstructorName(row)}
                      </td>
              ...
              {pagination.items.map((row) => (
                <article key={row._id} className="rounded-xl border border-(--line) bg-(--surface) p-3">
                  ...
                  <p className="mt-1 text-xs text-(--text-dim)">
                    Instructor: {resolveAcademicDepartmentInstructorName(row)}
                  </p>
                  <p className="text-xs text-(--text-dim)">Created: {formatDate(row.createdAt)}</p>
      ...
      <footer className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-(--line) pt-4 text-sm">
        <p className="text-(--text-dim)">
          Showing {pagination.items.length} of {pagination.total} departments
        </p>
