...
import type { InstructorTableRow } from "@/lib/utils/instructor/instructor-utils";
import {
...
type InstructorTableProps = {
  loading: boolean;
  rows: InstructorTableRow[];
  total: number;
  currentPage: number;
  totalPages: number;
  statusPendingKey: string | null;
  statusOverrides: Record<string, UserStatus>;
  onDetails: (row: InstructorTableRow) => void | Promise<void>;
  onStatusChange: (
    row: InstructorTableRow,
    ...
    <>
      <div className="mt-4">
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-(--line) p-8 text-center text-sm text-(--text-dim)">
            No instructor found for current filters.
          </div>
        ) : (
          <>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-3">{resolveInstructorFullName(row)}</td>
      ...
      <footer className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-(--line) pt-4 text-sm">
        <p className="text-(--text-dim)">
          Showing {rows.length} of {total} instructors
        </p>
