...
import type { InstructorTableProps } from "@/lib/type/dashboard/admin/instructor/ui";
import type { InstructorStatus } from "@/lib/type/dashboard/admin/instructor";
...
const statusOptions: InstructorStatus[] = ["active", "blocked"];
            ...
            {!loading && items.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-(--text-dim)">
                  {error ? "Failed to load instructors." : "No instructors found."}
                </td>
              </tr>
            ) : null}
                    ...
                    <td className="px-5 py-4">
                      {userId && canChangeStatus ? (
                        <select
                          value={currentStatus}
                          onChange={(event) =>
                            onStatusChange(item, event.target.value as InstructorStatus)
                          }
